...
class VIDEOSR(object):
    ...
    def forward(self, frames_lr, is_training=True, reuse=False):
        ...
        def RB(block_in, num_b=1, num_d=3, max_feature=64, scope='RB'):
            for i in range(num_b):
                block_out=block_in
                with tf.variable_scope('res_block', reuse=False) as res_block:
                    with tf.variable_scope(scope, reuse=False):
                        for j in range(num_d):
                            conv0=slim.conv2d(block_out, 64, [3, 3], scope='conv0_{}_{}'.format(i,j))
                            block_out=tf.concat([conv0,block_out],3)
                        block_out=slim.conv2d(block_out, 64, [1, 1], scope='out_{}'.format(i))
                        block_in+=block_out
            return block_in
        ...
        for i in range(num_frame):
            ...
            with tf.variable_scope('srmodel', reuse=reuse) as scope_sr:
                with slim.arg_scope([slim.conv2d], activation_fn=prelu , stride=1,
                    ...
                    block_in=conv1
                    block_in, rnn_state= MMB(block_in, cell, rnn_state, num_b=10, num_d=3, scope='MMB')
    ...
    def testvideo(self, dataPath=None, savename='result', reuse=False, scale_factor=4, num_frames=3):
        ...
        for idx0 in range(len(inList)):
            ...
            if len(dims) == 3:
                scipy.misc.imsave(os.path.join(DATA_TEST_OUT, 'mmvsr_%03d.png' % (idx0)),
...
def main(_):
    model = VIDEOSR()
    model.train()
    #model.testvideos('/dev/f/data/video/test/srmd5',0,'mmvsr')
